...
var RemoteCopilotCloud = class extends CopilotCloud {
  constructor(cloudApiUrl = import_shared4.COPILOT_CLOUD_API_URL) {
    super();
  ...
  async checkGuardrailsInput({
    ...
    const url = `${this.cloudApiUrl}/${import_shared4.COPILOT_CLOUD_VERSION}/copilotkit/guardrails/input`;
...
var CopilotRuntime = class {
  constructor(params) {
    ...
    for (const action of (params == null ? void 0 : params.actions) || []) {
      if ("argumentAnnotations" in action) {
        this.actions.push((0, import_shared5.annotatedFunctionToAction)(action));
      } else {
  ...
  removeBackendOnlyProps(forwardedProps) {
    const backendOnlyPropsKeys = forwardedProps[import_shared5.EXCLUDE_FROM_FORWARD_PROPS_KEYS];
    if (Array.isArray(backendOnlyPropsKeys)) {
      ...
      const success = Reflect.deleteProperty(forwardedProps, import_shared5.EXCLUDE_FROM_FORWARD_PROPS_KEYS);
      if (!success) {
  ...
  async getResponse(forwardedProps, serviceAdapter, publicApiKey) {
    ...
    const mergedTools = flattenToolCallsNoDuplicates([
      ...serversideTools.map(import_shared5.actionToChatCompletionFunction),
  ...
  async response(req, serviceAdapter) {
    const publicApiKey = req.headers.get(import_shared5.COPILOT_CLOUD_PUBLIC_API_KEY_HEADER) || void 0;
    try {
  ...
  async streamHttpServerResponse(req, res, serviceAdapter, headers) {
    ...
    const publicApiKey = (req.header ? (
      ...
      req.header(import_shared5.COPILOT_CLOUD_PUBLIC_API_KEY_HEADER)
    ) : (
      // use headers in node http
      req.headers[import_shared5.COPILOT_CLOUD_PUBLIC_API_KEY_HEADER.toLowerCase()]
    )) || void 0;
    const response = await this.getResponse(forwardedProps, serviceAdapter, publicApiKey);
